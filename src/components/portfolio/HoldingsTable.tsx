...
import { formatSAR, formatPct } from '@/lib/utils'
import type { HoldingWithLive } from '@/types'

export default function HoldingsTable() {
  // ✅ إصلاح: استخراج add هنا في أعلى الكومبوننت بدل داخل الدالة
  const { holdings, remove, add } = usePortfolioStore()
  ...
  const addHolding = () => {
    ...
    if (!t || !q || !p) return
    // ✅ يستخدم add المستخرج من أعلى الكومبوننت بشكل صحيح
          ...
          <div className="p-4 border-b border-b-1 bg-bg3 grid grid-cols-2 md:grid-cols-4 gap-3">
            {([
              ...
              <div key={k}>
                ...
                <input
                  ...
                  placeholder={ph}
                  type={k === 'qty' || k === 'price' ? 'number' : 'text'}
            ...
            <div className="col-span-2 md:col-span-4 flex justify-end gap-2">
              <button
                ...
                className="px-3 py-1.5 text-xs text-tx-3 hover:text-tx transition-colors"
              >
                إلغاء
              </button>
              <button
                onClick={addHolding}
                className="px-4 py-1.5 bg-gr text-bg text-xs font-bold rounded-lg hover:bg-gr/90 transition-colors"
              >
                إضافة
              </button>
            </div>
                    ...
                    <tr key={h.ticker}>
                      ...
                      <td>
                        <button
                          ...
                          className="text-rd hover:text-rd/70 text-xs transition-colors"
                        >
                          حذف
                        </button>
                      </td>
