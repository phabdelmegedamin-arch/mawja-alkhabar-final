...
export default function SignalBar({ result }: Props) {
  ...
  const verdicts = {
    pos: abs >= 75 ? 'إشارة صعود قوية جداً 🚀'
       : abs >= 50 ? 'إشارة صعود واضحة 📈'
       : 'ميل إيجابي ↗',
    neg: abs >= 75 ? 'إشارة هبوط حادة ⚠️'
       : abs >= 50 ? 'إشارة هبوط واضحة 📉'
       : 'ميل سلبي ↘',
    ...
    >
      ...
      <div className="h-1.5 bg-bg3">
        <div
          ...
          style={{ width: `${Math.max(8, abs)}%`, background: col }}
        />
      </div>
      ...
      {topStocks.length > 0 && (
        <div className="flex flex-wrap gap-2 p-3">
            ...
            return (
              <div
                ...
                className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg
                           bg-bg3 border border-b-2 text-xs"
              >
                <span className="font-mono font-bold text-ac">{s.t}</span>
                <span className="text-tx-2">{s.n}</span>
                <span className="font-mono font-bold" style={{ color: pColor }}>
                  {s.pct}
                </span>
                <button
                  onClick={() => inW ? remove(s.t ?? '') : add(s.t ?? '', s.n ?? '')}
                  className="ml-0.5 transition-all hover:scale-110"
          ...
          {stocks.length > 5 && (
            <span className="px-2.5 py-1.5 text-xs text-tx-3">
              +{stocks.length - 5} سهم آخر
            </span>
          )}
        </div>
      )}
