...
export default function NewsList() {
  ...
  const stats = { pos: items.filter(i => i.sentiment?.dir==='pos').length, neg: items.filter(i => i.sentiment?.dir==='neg').length, neu: items.filter(i => i.sentiment?.dir==='neu').length }
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex gap-2 flex-wrap">
        ...
        {(['all','pos','neg','neu'] as const).map(f => (
          <button key={f} onClick={() => setFilter(f)}
            className={cn('px-3 py-2 rounded-lg text-xs font-bold transition-colors',
              filter===f ? (f==='pos'?'bg-gr text-bg':f==='neg'?'bg-rd text-bg':f==='neu'?'bg-yl text-bg':'bg-ac text-bg')
      ...
      {!loading && filtered.map((item, i) => {
        ...
        return (
          <div key={i} className={cn('card p-4 cursor-pointer hover:border-b-3 transition-all duration-200',
            dir==='pos'?'hover:border-gr/40':dir==='neg'?'hover:border-rd/40':'hover:border-yl/40')}
            style={{ borderRight: `3px solid ${col}20` }}
            onClick={() => { if (typeof window !== 'undefined') { const url = new URL('/', window.location.origin); sessionStorage.setItem('mw_news_text', item.text); window.location.href = '/' } }}>
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1 flex-wrap">
                  ...
                  <span className="text-2xs text-tx-3 mr-auto">{timeAgo(item.pubDate || item.fetchedAt)}</span>
                ...
                {item.desc && <div className="text-xs text-tx-3 leading-relaxed line-clamp-2">{item.desc}</div>}
                <div className="mt-2 h-1 bg-bg3 rounded-full overflow-hidden">
                  <div className="h-full rounded-full transition-all duration-700" style={{ width: `${Math.min(abs,100)}%`, background: col }} />
                </div>
              </div>
            </div>
          </div>
