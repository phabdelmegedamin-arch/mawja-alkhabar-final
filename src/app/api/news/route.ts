...
// ─── فلترة محسّنة: تبحث بكل الكلمات المفتاحية المتاحة ───
async function fetchStockNews(
  ...
  try {
    ...
    // جمع كل الكلمات المفتاحية الممكنة
            ...
            <div style={{ marginTop: '10px' }}>
              <div style={{
                ...
                {stockNews.map((item, idx) => (
                  <button
                    key={idx}
                    onClick={() => handleSelectNewsItem(idx, stockNews)}
                    style={{
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', gap: '10px', fontSize: '0.7rem', color: 'var(--t2)', alignItems: 'center' }}>
                      ...
                      {item.pubDate && (
                        <span>🕐 {(() => { try { return new Date(item.pubDate).toLocaleDateString('ar-SA') } catch { return item.pubDate } })()}</span>
                      )}
                      {selectedNewsIdx === idx && (
                        <span style={{ color: '#00E5FF', fontWeight: 600, marginRight: 'auto' }}>✓ محدد للتحليل</span>
                      )}
                    </div>
                  </button>
                ))}
