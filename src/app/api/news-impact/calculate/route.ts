...
// المرحلة (ج): يستخدم بيانات Supabase عبر network-loader
// الحالة: استبدال نهائي للملف 03
// ══════════════════════════════════════════════════════════════════
 
import { NextRequest, NextResponse } from 'next/server'
import { classifyNews, compareScenarios } from '@/lib/news-impact-engine'
...
// ═══════════════════════════════════════════════════════
// محرك الحساب المحدّث — يستخدم بيانات Supabase
...
async function calculateWithSupabase(params: NewsImpactParams) {
  ...
  const classification = newsText
    ...
    : { type: params.newsType ?? 'GENERAL', name_ar: newsTypes[params.newsType ?? 'GENERAL']?.name_ar ?? 'خبر عام', suggestedS:1.0, confidence:0.3, direction:'NEUTRAL' as const, lambda:1.0 }
  ...
  while (queue.length > 0) {
    ...
    for (const rel of rels) {
      ...
      const effectiveOwn   = cumOwn * ownershipRatio * (LAYER_DECAY[rel.layer] ?? 0.25)
